...
def _convert_cond_effect(mapping, eff):
    if isinstance(eff, And):
        # WARNING: This line used to filter out object equality, but it was ruling out legit conditional effects
        #  with an equality in the condition. The original line is commented out below.
        #return [_create_cond(mapping, f) for f in filter(lambda x: '=' not in str(x), eff.args)]
        #
        return [_create_cond(mapping, f) for f in eff.args]
...
if __name__ == '__main__':
    ...
    # Check to make sure we didn't split on disjunctive preconditions -- would make the validation fail
    with open(sol, 'r') as f:
        if "__or_variant" in f.read():
            print ("\n\tError: Cannot validate problems with disjunctive preconditions.\n\n")
            os.sys.exit(1)
